...
public class Grid {
	...
	public void genGrid(int numberOfObstacles, int numberOfPads){
		...
		for (int i = 0; i < numberOfObstacles; i++){
			...
			while(checkObstacleBlockage(obstaclePositionX, obstaclePositionY)){
				obstaclePositionX = (int) Math.floor(Math.random()*this.width);
				obstaclePositionY = (int) Math.floor(Math.random()*this.height);
			}
		...
		for (int i = 0; i < numberOfPads; i++){
			...
			while (this.cells[padPositionX][padPositionY].hasRock || !(this.cells[padPositionX][padPositionY].status == CellStatus.free)){
...
//			while (!(this.cells[rockPositionX][rockPositionY].status == CellStatus.free || !this.cells[rockPositionX][rockPositionY].hasRock)){
...
//			this.cells[rockPositionX][rockPositionY].hasRock = true;
	...
	public boolean checkObstacleBlockage(int x, int y){
		boolean c = false;
		boolean d = false;
//			a = (this.cells[x+1][y].status == CellStatus.obstacle || this.cells[x-1][y].status == CellStatus.obstacle); 
//			b = (this.cells[x][y+1].status == CellStatus.obstacle || this.cells[x][y-1].status == CellStatus.obstacle); 
		if (!(x == 0 || x == width-1 || y == 0 || y == height-1)){
				c = (this.cells[x+1][y+1].status == CellStatus.obstacle || this.cells[x+1][y-1].status == CellStatus.obstacle); 
				d = (this.cells[x-1][y-1].status == CellStatus.obstacle || this.cells[x-1][y-1].status == CellStatus.obstacle);
		}
		return (c || d);
	}
